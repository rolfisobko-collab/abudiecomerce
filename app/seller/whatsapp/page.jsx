...
const WhatsAppChat = () => {
  ...
  useEffect(() => {
    if (selectedChat) {
      const fetchMessages = async () => {
        try {
          const response = await fetch(`/api/seller/whatsapp/messages/${selectedChat.contact_wa_id}`)
          const data = await response.json()
          if (data.success) {
  ...
  return (
    <div className="relative flex h-screen bg-gray-100 w-full">
      {/* Contenido principal con blur */}
      <div className="flex w-full filter blur-sm pointer-events-none">
        {/* Lista de chats */}
      ...
      {/* Overlay con cartel de desarrollo */}
      <div className="absolute inset-0 flex items-center justify-center bg-black/20 backdrop-blur-sm">
        <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md mx-4 text-center border border-gray-200">
          ...
          <p className="text-gray-600 mb-6 leading-relaxed">
            El sistema de WhatsApp Business está siendo desarrollado. 
            Pronto podrás gestionar conversaciones y ver mensajes de tus clientes.
          </p>
